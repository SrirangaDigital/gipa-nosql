...
<?php foreach ($data['articles'] as $article) { ?>
            ...
            <div class="full-width-card blue-edge">
                <h4 class="publication-details">
                    ...
                    <span class="gray"><a href="<?=BASE_URL?>articles/toc?volume=<?=$article['volume']?>&issue=<?=$article['issue']?>"><?=ARCHIVE_VOLUME?> <?=$viewHelper->rlZero($article['volume'])?>, <?=ARCHIVE_ISSUE?> <?=$viewHelper->rlZero($article['issue'])?>, (<?=ARCHIVE_YEAR?> <?=$viewHelper->rlZero($article['year'])?>, <?=ARCHIVE_MONTH?> <?=$viewHelper->rlZero($article['month'])?>)</a></span>
                </h4>

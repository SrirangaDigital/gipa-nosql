...
class dataModel extends Model {
	...
	public function xml2Json() {
		...
		$xml = simplexml_load_file(PHY_METADATA_URL . GIPA . '/gipa.xml');
		...
		foreach ($xml->volume as $volume) {
			...
			foreach ($volume->issue as $issue) {
				...
				foreach ($issue->entry as $entry) {
					...
					$completeIssue['id'] = GIPA . '/' . $completeIssue['volume'] . '/' . $completeIssue['issue'];
					...
					$jsonFilePath = PHY_METADATA_URL . GIPA . '/' . $completeIssue['volume'] . '/' . $completeIssue['issue'] . '/';
	...
	public function insertEntries($collection) {
		...
		$titleAlphabet = [];
		$authorAlphabet = [];
		$jsonFiles = $this->getFilesIteratively(PHY_METADATA_URL . GIPA  , $pattern = '/index.json$/i');
		...
		foreach ($jsonFiles as $jsonFile) {
			...
			foreach ($content['toc'] as $article) {
				...
				if(isset($article['author'])) {
					...
					array_push($authorAlphabet, preg_replace('/(^.).*/', '$1', $author['name']));
				}

				// fetching initial letter from title
				array_push($titleAlphabet, preg_replace('/(^.).*/', '$1', $article['title']));
			}
		}
